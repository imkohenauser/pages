...
export class ArchRenderer {
  ...
  private readonly maskGeometry: ShapeGeometry;
  private readonly maskMesh: Mesh<ShapeGeometry, MeshBasicMaterial>;
  private readonly horseGeometry: PlaneGeometry;
  ...
  constructor(canvas: HTMLCanvasElement, host: HTMLElement, horseImage: HTMLImageElement) {
    ...
    const texture = new Texture(horseImage);
    ...
    this.contextLostHandler = (event: Event) => {
      ...
      this.stop(false);
      this.host.toggleAttribute('data-arch-scene-fallback', true);
    };
  ...
  private updateFrameAttribute() {
    for (let index = this.runs.length - 1; index >= 0; index -= 1) {
      ...
      if (step !== undefined) {
        this.host.dataset.archSceneFrame = String(step + 1);
        return;
      }
    }
    this.host.removeAttribute('data-arch-scene-frame');
